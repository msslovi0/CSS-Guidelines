...
This is forked from [csswizardy](https://github.com/csswizardry/CSS-Guidelines) and adapted to our needs.
...
Exceptions may be gradient syntax and URLs in comments. However, long,
comma-separated property values - such as collections of gradients or 
shadows - can be arranged across multiple lines in an effort to improve
readability and produce more useful diffs. 

Configure your editor to "show invisibles". This will allow you to
eliminate end of line whitespace, eliminate unintended blank line 
whitespace, and avoid polluting commits.

I prefer tabs over spaces and I write multi-line CSS.

### One file vs. many files

Some people prefer to work with single, large files. This is fine, and by
sticking to the following guidelines you'll encounter no problems.  
I might move to Sass one day which means sharding my stylesheets out into 
lots of tiny includes. This too is fine… Whichever method you choose, the 
following rules and guidelines apply. The only notable difference is with 
regards our table of contents and our section titles. Read on for further
explanation…
...
In this example it makes more sense to single-line our CSS.

## Naming conventions

I simply use hyphen delimited classes (e.g. `.foo-bar`, not `.foo_bar` or `.fooBar`).

Always ensure classes are sensibly named; keep them as short as possible but as long 
as necessary. Ensure any objects or abstractions are very vaguely named (e.g. `.ui-list`,
`.media`) to allow for greater reuse. Extensions of objects should be much more explicitly
named (e.g. `.user-avatar-link`). Don't worry about the amount or length of classes; 
gzip will compress well written code _incredibly_ well.
...
I only use pixels for items whose dimensions were defined before they came into
...
full explanation of how this works please refer to the article
...
You find a specifity calculator at [specificity.keegan.st](http://specificity.keegan.st/)
...
Selectors as a whole should be kept short but the class names themselves should
be as long as they need to be. A class of `.user-avatar` is far nicer than `.usr-avt`.

**Remember:** classes are neither semantic nor insemantic; they are sensible or
...
However, don't overdo classes in your markup. Throw in a class only where it is necessary!
...
IE stylesheets should be avoided. The only time an IE stylesheet may be required
is to circumvent blatant lack of support (e.g. PNG fixes). Most of the time we
will be able to work around IE problems with the help of classes attached to the
<html> element.
